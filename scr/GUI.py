...
def Url():
    ...
    errors = download(folderPath.get(),links(link),title)
    if errors != "":
        message.delete("1.0", 'end')
        message.insert(tk.END, errors, 'center')
...
button = tk.Button(window, text='Download Playlist', width=25, command=Url)
...
message.insert(tk.END,"\nThe entry box is for the playlist URL. It will download up to 101 videos\n\n"
                                            "Enter link in http:// or https:// format", 'center')
